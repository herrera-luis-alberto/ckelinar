...
EarthTrajectory NullTrajectoryForecaster::getTrayectory(
    const EarthPoint4D &begin, float upSpeed, float deltaT, float sigmaNoise) {
...
EarthTrajectory TrajectoryForecaster::getTrayectory(
    const EarthPoint4D &begin, float upSpeed, float deltaT, float sigmaNoise) {
  EarthTrajectory result;
  result.push_back( begin );
  ...
  while ( iterations < 1000 ) {
    ...
    if ( boost::math::isnan(post.latitude) ||
         boost::math::isnan(post.longitude) ) {
        break;
      }
...
bool TrajectoryForecaster::iterationStep(
    const EarthPoint4D &begin, EarthPoint4D &end, float upSpeed, float deltaT,
    float sigmaNoise) {
