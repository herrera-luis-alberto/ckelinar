...
class TrajectoryForecasterInterface {
public:
  virtual ~TrajectoryForecasterInterface() {};
  ...
  virtual EarthTrajectory getTrayectory(const EarthPoint4D &begin,
                                        float upSpeed, float deltaT,
                                        float sigmaNoise=0) = 0;
};

class NullTrajectoryForecaster : public TrajectoryForecasterInterface {
public:
  virtual EarthTrajectory getTrayectory(const EarthPoint4D &begin,
                                        float upSpeed, float deltaT,
                                        float sigmaNoise=0);
...
class TrajectoryForecaster : public TrajectoryForecasterInterface {
public:
...
protected:
  ...
  bool iterationStep( const EarthPoint4D &begin, EarthPoint4D &end,
                      float upSpeed, float deltaT, float sigmaNoise);
  ...
  bool getArrayIndex(const EarthPoint4D &point, int *latIndex, int *lonIndex,
                     int *heightIndex, int *timeIndex);
  ...
  static boost::const_multi_array_ref<float, 2> *read2DimVar(
      const NcFile &data, const string& name, float* buffer);

  static boost::const_multi_array_ref<float, 4> *read4DimVar(
      const NcFile &data, const string& name, float* buffer);

  /**
   * bilinearInterpolation generates an interpolation of a point in between
   ...
   *------&------------------------------&
   */
  float bilinearInterpolation(float latitude, float longitude,
                              boost::const_multi_array_ref<float, 4> *data,
                              int t, int h, int latIndex, int lonIndex);
};
